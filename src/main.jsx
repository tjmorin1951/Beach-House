...
async function sendInvites(bk){try{const list=(bk.invitees||[]).filter(iv=>iv&&iv.email);if(!list.length)return;const signedIn=(auth&&auth.currentUser&&auth.currentUser.email?auth.currentUser.email:'').toLowerCase();await fetch('/.netlify/functions/send-invite',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({invitees:list,organizerName:fullName(bk),organizerEmail:(bk.email||'').trim()||signedIn,startDate:prettyDate(bk.startDate),endDate:prettyDate(bk.endDate),nights:nightsCount(bk.startDate,bk.endDate)})});}catch(e){console.error('Invite email failed:',e);}}
async function sendConfirmation(bk){try{const signedIn=(auth&&auth.currentUser&&auth.currentUser.email?auth.currentUser.email:'').toLowerCase();const guest=(bk.email||'').trim().toLowerCase();const to=[...new Set([signedIn,guest].filter(Boolean))];if(!to.length)return;const roomNames=(bk.rooms||[]).map(rid=>ROOMS.find(r=>r.id===rid)?.name).filter(Boolean);await fetch('/.netlify/functions/send-confirmation',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to,guestName:fullName(bk),startDate:prettyDate(bk.startDate),endDate:prettyDate(bk.endDate),rawStart:bk.startDate,rawEnd:bk.endDate,bookingId:bk.id,nights:nightsCount(bk.startDate,bk.endDate),numGuests:bk.numGuests,rooms:roomNames})});}catch(e){console.error('Confirmation email failed:',e);}}
...
async function sendJoinRequestEmail(req,target){try{const to=[...new Set([(target.createdBy||'').trim().toLowerCase(),(target.email||'').trim().toLowerCase()].filter(Boolean))];if(!to.length)return;const roomNames=(req.rooms||[]).map(rid=>ROOMS.find(r=>r.id===rid)?.name).filter(Boolean);await fetch('/.netlify/functions/send-join-request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to,hostName:fullName(target),requesterName:fullName(req),requesterEmail:req.email||'',requesterPhone:req.phone||'',startDate:prettyDate(req.startDate),endDate:prettyDate(req.endDate),nights:nightsCount(req.startDate,req.endDate),numGuests:req.numGuests,rooms:roomNames,message:req.joinMessage||''})});}catch(e){console.error('Join request email failed:',e);}}
...
export default function App() {
  ...
  const handleSave=async bk=>{const exists=bookings.some(b=>b.id===bk.id);const updated=exists?bookings.map(b=>b.id===bk.id?bk:b):[...bookings,bk];const sorted=[...updated].sort((a,b)=>a.startDate.localeCompare(b.startDate));setBookings(sorted);setEditingBooking(null);setView('home');const ok=await saveAllBookings(sorted);if(ok&&!exists){sendConfirmation(bk);sendInvites(bk);}showToast(ok?(editingBooking?'Stay updated ✨':'Booked! See you at the beach 🌊'):'Could not save — check your connection.',ok?'success':'error');};
  const handleSaveJoinRequest=async req=>{const updated=[...joinRequests,req];setJoinRequests(updated);setEditingBooking(null);setView('home');const ok=await saveAllRequests(updated);if(ok){const t=bookings.find(b=>b.id===req.targetBookingId);if(t)sendJoinRequestEmail(req,t);}showToast(ok?`Join request sent to ${req.targetGuestName}! 🤝`:'Could not save.',ok?'success':'error');};
...
function WholeHousePicker({wholeHouse,setWholeHouse,conflictedRooms,datesValid,codeUnlocked,onSubmitCode}){
  ...
  if(!wholeHouse) return(
    <button onClick={()=>{if(!datesValid)return;setWholeHouse(true);onSubmitCode();}} disabled={!datesValid} className="w-full text-left rounded-2xl p-5 grain mb-1 transition-all hover:scale-[1.01] relative overflow-hidden" style={{background:'linear-gradient(135deg,#1B4965,#1F3A4E)',color:'#FBF6EE',opacity:datesValid?1:0.5,cursor:datesValid?'pointer':'not-allowed'}}>
...
function StayRow({booking,onEdit,onDelete}){
  const [confirming,setConfirming]=useState(false);const userEmail=(auth&&auth.currentUser&&auth.currentUser.email?auth.currentUser.email:'').toLowerCase();const canModify=!booking.createdBy||booking.createdBy===userEmail||userEmail==='tjmorin74@gmail.com'||userEmail==='jamorin1@charter.net';
